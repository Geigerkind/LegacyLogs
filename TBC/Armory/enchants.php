...
$template = array();
foreach($pdo->query('SELECT column_1 as Field01, column_2 as Field02, column_3 as Field03, column_5 as Field05, column_8 as Field08, column_11 as Field11, column_12 as Field12, column_14 as Field14 FROM enchantmentdbc_tbc') as $row){
	$template[$row["Field01"]] = $row;
}

$pdo->query('DELETE FROM `armory-enchantmentstats-tbc`');

// translate dbc
$sql = 'INSERT INTO `armory-enchantmentstats-tbc` (`enchid`, `name`, `stamina`, `strength`, `intellect`, `spirit`, `agility`, `apower`, `dodge`, `blockchance`, `block`, `spellpower`, `healpower`, `hit`, `crit`, `defense`, `spellhit`, `spellcrit`, `res_nature`, `res_holy`, `res_fire`, `res_frost`, `res_shadow`, `res_arcane`, `manaregen`, `mana`, `health`, `armor`, `haste`, `spellhaste`, `spellpen`, `resilience`, `expertise`) VALUES ';
$insertQuery = array();
$insertData = array();
...
foreach($template as $k => $v){
	if (intval($k)!=0){
		...
		$insertData[] = $template["spellhaste"];
