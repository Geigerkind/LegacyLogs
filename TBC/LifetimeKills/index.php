	...
	public function content(){
		...
		<div class="container cont" id="container">
			...
			$content .= '
				...
				<a href="{path}TBC/LifetimeKills/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($this->page+1).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
				<a href="{path}TBC/LifetimeKills/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($r = (($this->page-1)>0) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
			</section>
			<section class="table">
				<div class="right">
					<table cellspacing="0">
		...
		foreach($this->db->query('SELECT b.name, c.name as gname, d.name as sname, a.lhk, e.seen, b.faction, b.classid, e.gender, e.race FROM `armory-honor-tbc` a LEFT JOIN `chars` b ON a.charid = b.id LEFT JOIN guilds c ON b.guildid = c.id LEFT JOIN servernames d ON b.serverid = d.id LEFT JOIN `armory` e ON a.charid = e.charid'.$this->getConditions().' ORDER BY a.lhk DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
			$fac = ($row->faction==1) ? "alliance" : "horde";
			$content .= '
						<tr>
							<td class="rank">'.$num.'</td>
							<td class="timg char"><img src="{path}Database/racegender/Ui-charactercreate-races_'.$this->race[$row->race].'-'.$this->gender[$row->gender].'-small.png" /><img src="img/c'.$row->classid.'.png"><a href="{path}TBC/Character/'.$row->sname.'/'.$row->name.'/0" class="color-'.$this->classById[$row->classid].'">'.$row->name.'</a></td>
							<td class="timg"><img src="img/'.$fac.'.png"><a class="'.$fac.'" href="{path}TBC/Guild/'.$row->sname.'/'.$row->gname.'/0">'.$row->gname.'</a></td>
