...
class Home extends Site{
	...
	public function content(){
		...
		';
		$count = $this->db->query('SELECT a.id FROM `tbc-raids` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN `tbc-raids-participants` c ON a.id = c.rid LEFT JOIN `tbc-raids-attempts` d ON a.id = d.rid LEFT JOIN tbc_npcs e ON d.npcid = e.id WHERE (c.dps LIKE "%'.$this->info->id.'%" or c.healers LIKE "%'.$this->info->id.'%" or c.tanks LIKE "%'.$this->info->id.'%") AND a.rdy = 1 AND e.type=1 AND d.type=1')->rowCount();
		$max = floor($count/15+1);
		$i = 0;
		foreach ($this->db->query('SELECT d.time, a.nameid, b.name, a.id, e.name as boss, d.npcid FROM `tbc-raids` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN `tbc-raids-participants` c ON a.id = c.rid LEFT JOIN `tbc-raids-attempts` d ON a.id = d.rid LEFT JOIN tbc_npcs e ON d.npcid = e.id WHERE (c.dps LIKE "%'.$this->info->id.'%" or c.healers LIKE "%'.$this->info->id.'%" or c.tanks LIKE "%'.$this->info->id.'%") AND a.rdy = 1 AND e.type=1 AND d.type=1 ORDER BY d.time DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
			$content .= '
						<tr>
		...
		$content .= '
			...
			<footer class="pager">
				<div class="pleft">
					<a href="{path}TBC/Character/Kills/'.$this->sname.'/'.$this->name.'/1"><button class="icon-button icon-darrowleft"></button></a><a href="{path}TBC/Character/Kills/'.$this->sname.'/'.$this->name.'/'.($r = (($this->page-1)>=1) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft"></button></a>
				</div>
				<div class="pright">
					<div class="pseudoButton">Page '.$this->page.' of '.$max.'</div><a href="{path}TBC/Character/Kills/'.$this->sname.'/'.$this->name.'/'.($r = (($this->page+1)<=$max) ? ($this->page+1) : $max).'"><button class="icon-button icon-arrowright"></button></a><a href="{path}TBC/Character/Kills/'.$this->sname.'/'.$this->name.'/'.$max.'"><button class="icon-button icon-darrowright"></button></a>
				</div>
			</footer>
		</div>
		';
