	...
	public function content(){
		...
		<div class="container cont" id="container">
			...
			$content .= '
				...
				<a href="{path}TBC/2vs2/?server='.$this->server.'&faction='.$this->faction.'&page='.($this->page+1).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
				<a href="{path}TBC/2vs2/?server='.$this->server.'&faction='.$this->faction.'&page='.($r = (($this->page-1)>0) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
			</section>
			<section class="table">
				<div class="right">
					<table cellspacing="0">
		...
		foreach($this->db->query('SELECT a.*, b.name as sname, group_concat(d.classid) as member, group_concat(d.name) as membernames, group_concat(d.faction) AS faction FROM `armory-arenateams-tbc` a LEFT JOIN servernames b ON a.serverid = b.id LEFT JOIN `armory-honor-tbc` c ON c.arena1 = a.arenaid LEFT JOIN chars d ON d.id = c.charid WHERE c.arena1 = a.arenaid AND a.type=0'.$this->getConditions().' GROUP BY a.arenaid ORDER BY a.rating DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
			$row->faction = explode(",",$row->faction)[0];
			$fac = ($row->faction==1) ? "alliance" : "horde";
			$content .= '
						<tr>
							<td class="rank">'.$num.'</td>
							<td class="timg"><img src="img/'.$fac.'.png"><a href="{path}TBC/Team/'.$row->sname.'/'.$row->name.'">'.$row->name.'</a></td>
							<td class="lstring char">
			';
			$memNames = explode(",", $row->membernames);
			foreach(explode(",", $row->member) as $memKey => $mem){
				$content .= '<a href="{path}TBC/Armory/'.$row->sname.'/'.$memNames[$memKey].'/0"><img src="{path}Database/classes/c'.$mem.'.png" title="'.$memNames[$memKey].'"/></a>';
			}
			$content .= '
							</td>
