...
class Home extends Site{
	...
	public function content(){
		...
		$content .= '
				...
				<a href="{path}TBC/Immortal-Runs/Ranking/?server='.$this->server.'&faction='.$this->faction.'&mode=0&id='.($r = ($this->rid>14) ? ($this->rid-1) : $this->rid).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
				<a href="{path}TBC/Immortal-Runs/Ranking/?server='.$this->server.'&faction='.$this->faction.'&mode=0&id='.($r = ($this->rid<24) ? ($this->rid+1) : $this->rid).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
			</section>
			<section class="table">
				<div class="right">
					<table cellspacing="0">
		...
		';
		$counter = 0;
		foreach ($this->db->query('SELECT a.'.$this->prefix[1].'deaths as deaths, a.'.$this->prefix[1].'change as changed, a.'.$this->prefix[2].'rid as raidid, c.name as sname, b.name as gname, d.tsend, b.serverid, b.faction, a.'.$this->prefix[1].'amount as aamount FROM `tbc-immortal-runs` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN servernames c ON b.serverid = c.id LEFT JOIN `tbc-raids` d ON a.'.$this->prefix[2].'rid = d.id WHERE a.raidnameid = '.$this->rid.' '.$this->getConditions().' ORDER BY a.'.$this->prefix[1].'deaths LIMIT 50') as $row){
