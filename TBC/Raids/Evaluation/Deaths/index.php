...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT b.deathid, a.attemptid, a.flagid as kblower, a.flag kblowerflag, b.id, b.flagid, b.flag, b.dmg, b.heal, b.type, b.abilityid, b.cbt, c.time FROM `tbc-raids-deaths` a LEFT JOIN `tbc-raids-individual-death` b ON a.id = b.deathid LEFT JOIN `tbc-raids-attempts` c ON a.attemptid = c.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND c.rdy = 1 AND '. $con) as $row){
			...
			$offset = explode(",", $this->raidinfo->deaths);
			foreach($this->db->query('SELECT a.id, c.id AS attemptid, a.killingblow, a.cbt, b.abilityid, b.dmg, b.heal, b.id as did, b.charid, a.flagid FROM `tbc-raids-deaths` a JOIN `tbc-raids-individual-death` b ON a.id = b.deathid LEFT JOIN `tbc-raids-attempts` c ON a.attemptid = c.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND c.rdy = 1 AND '.$con.' ORDER BY a.cbt, b.id') as $row){
	...
	public function content(){
		...
		foreach($var as $keys => $vars){
			...
			$content .= '
							<tr>
								...
								<td class="vstring"><img src="{path}Database/icons/small/'.$this->spells[$vars->abilityid]->icon.'.jpg" /><a rel="spell='.$vars->abilityid.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$vars->abilityid.'">'.$this->spells[$vars->abilityid]->name.'</a></td>
				...
				foreach($va as $key => $var){
					...
					$content .= '
							<tr>
								...
								<td class="vvstring"><img src="{path}Database/classes/c'.$this->participants[4][$key]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$key]->classid].'" href="{path}TBC/Raids/Evaluation/Deaths/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$key.'&sel='.$this->sel.'">'.$this->participants[4][$key]->name.'</a></td>
								...
								<td class="sstring"><img src="{path}Database/icons/small/'.$this->spells[$abilities[1]]->icon.'.jpg" /><a target="_blank" rel="spell='.$abilities[1].'" href="https://tbc-twinhead.twinstar.cz/?spell='.$abilities[1].'">'.$this->spells[$abilities[1]]->name.'</a></td>
								<td class="num">'.($abilities[7]-$abilities[6]).'s</td>
								<td class="lstring">
					';
					$next = false;
					foreach($abilities[2] as $keys => $vars){
						$content .= ($r = ($next) ? ", " : "").'<a target="_blank" rel="spell='.$vars.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$vars]->id.'">'.$this->spells[$vars]->name.'</a>';
						$next = true;
					}
					$content .= '
								</td>
		...
		foreach($this->byAmount as $k => $v){
			$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								<td class="char"><img src="{path}Database/classes/c'.$this->participants[4][$k]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$k]->classid].'" href="{path}TBC/Raids/Evaluation/Deaths/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$k.'&sel='.$this->sel.'">'.$this->participants[4][$k]->name.'</a></td>
