...
class Home extends Template{
	...
	private function getTable(){
		...
		switch ($this->mode){
			case 0 :
				...
				foreach ($this->db->query('SELECT a.charid, a.amount, a.active FROM `tbc-raids-individual-dmgdonetoenemy` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$t[1] += $row->amount;
			...
			case 1 :
				$offset = explode(",", $this->raidinfo->indddte);
				foreach ($this->db->query('SELECT a.npcid, a.amount, a.active, a.attemptid FROM `tbc-raids-individual-dmgdonetoenemy` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$t[1] += $row->amount;
			...
			case 2 : 
				...
				foreach ($this->db->query('SELECT a.abilityid, a.amount, a.casts, a.hits, a.crit, a.miss, a.parry, a.dodge, a.resist FROM `tbc-raids-individual-dmgdonebyability` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2.' ORDER BY a.casts') as $row){
					$t[1] += $row->amount;
			...
			case 3 :
				...
				foreach ($this->db->query('SELECT a.charid, a.amount, a.culpritid FROM `tbc-raids-individual-dmgtakenbyplayer` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
					if ($this->player){
						$t[1] += $row->amount;
		...
		if ($this->mode != 2){
			...
			foreach ($this->db->query('SELECT a.id, a.charid, a.realm, a.charid, a.realmtype, a.realmclass, b.npcid FROM `tbc-raids-records` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND a.type = -1 AND '.$con.' ORDER BY a.realm, a.realmtype, a.realmclass LIMIT 12') as $row){
				$t[10][$row->id] = $row;
			}
		}
		return $t;
	...
	private function getGraphValues(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `tbc-raids-newbuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `tbc-raids-newdebuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			if ($this->tarid){
				if ($this->mode==3){
					...
					foreach ($this->db->query('SELECT a.time, a.amount, a.charid, a.abilityid FROM `tbc-raids-graph-individual-friendlyfire` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND culpritid = "'.$this->player.'"') as $row){
					...
					foreach ($this->db->query('SELECT a.time, a.amount, a.npcid, a.abilityid FROM `tbc-raids-graph-individual-dmgdone` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
					return $this->evalStackedGraphValues($q, false, $buffs, $debuffs);
				...
				if ($this->mode>1 && $this->mode!=3){
					foreach ($this->db->query('SELECT a.time, a.amount, a.charid, a.abilityid FROM `tbc-raids-graph-individual-dmgdone` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
					return $this->evalStackedGraphValues($q, false, $buffs, $debuffs);
			...
			$offset = explode(",", $this->raidinfo->graphdmg);
			if ($this->mode==2){
				foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid FROM `tbc-raids-graph-individual-dmgdone` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$q[$row->abilityid][1] .= $row->time.",";
					$q[$row->abilityid][2] .= $row->amount.",";
				}
				return $this->evalStackedGraphValues($q);
			}elseif ($this->mode==1){
				foreach ($this->db->query('SELECT a.time, a.amount, a.npcid FROM `tbc-raids-graph-individual-dmgdone` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
				...
				$offset = explode(",", $this->raidinfo->graphff);
				foreach ($this->db->query('SELECT a.time, a.amount, a.culpritid FROM `tbc-raids-graph-individual-friendlyfire` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.charid FROM `tbc-raids-graph-individual-dmgdone` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
			...
			foreach($p as $ke => $va){
				foreach($va as $key => $var){
					if ($space > 1){
						...
						foreach($var as $k => $v){
							for ($i=1; $i<=50; $i++){
								if ($k<=($i*$space+$low) && $k>=(($i-1)*$space+$low) && $k<=20000 && $v<=100000){
