...
class Home extends Template{
	...
	private function getTable(){
		...
		foreach($this->db->query('SELECT * FROM `tbc-raids-newdebuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND '.$con.'rid = '.$this->rid) as $row){
		...
		foreach($p as $k => $v){
			...
			foreach($this->db->query('SELECT a.abilityid, a.amount FROM `tbc-raids-individual-debuffsbyplayer` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.amount') as $row){
