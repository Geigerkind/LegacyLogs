...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT a.abilityid, a.amount, a.tarid, a.attemptid FROM `tbc-raids-casts` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.amount DESC') as $row){
			...
			$offset = explode(",", $this->raidinfo->casts);
			foreach($this->db->query('SELECT a.abilityid, a.amount FROM `tbc-raids-casts` a LEFT JOIN `tbc-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.amount DESC') as $row){
	...
	public function content(){
		...
		foreach ($table[4] as $key => $var){
			$content .= '
							<tr>
								...
								<td><img src="{path}Database/icons/small/'.$table[6][$key][2].'.jpg" /><a rel="spell='.$key.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'">'.$table[6][$key][1].'</a></td>
		...
		foreach ($table[5] as $key => $var){
			$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								<td><img src="img/c0.png" /><a href="{path}TBC/Raids/Evaluation/Casts/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$this->player.'&sel='.$table[9][$key].'&pet='.$this->pet.'&tarid='.$this->tarid.'&mode='.$this->mode.'">'.$table[8][$key][3].'</a></td>
								<td>'.round(100*$var/$table[7], 2).'%</td>
								<td><div class="amount-sbar border-round"><div class="amount-sbar-progress bgcolor-" style="width: '.(100*$var/$table[7]).'%;"></div></div></td>
								<td>'.$this->mnReadable($var).'</td>
							</tr>
			...
			foreach($table[1] as $key => $var){
				foreach($var as $k => $v){
					$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								<td><img src="{path}Database/icons/small/'.$table[3][$key][$k][2].'.jpg" /><a rel="spell='.$key.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'">'.$table[3][$key][$k][1].'</a></td>
								<td><img src="img/c0.png" /><a href="{path}TBC/Raids/Evaluation/Casts/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$this->player.'&sel='.$table[2][$key][$k].'&pet='.$this->pet.'&tarid='.$this->tarid.'&mode='.$this->mode.'">'.$table[3][$key][$k][3].'</a></td>
		...
		';
			...
			foreach ($table[1][3] as $key => $var){
				$content .= '
					<tr>
						<td class="count">'.$count.'</td>
						<td><img src="{path}Database/icons/small/'.$table[3][3][$key][2].'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'">'.$table[3][3][$key][1].'</a></td>
						<td>'.$this->mnReadable($var).'</td>
		...
		';
			...
			foreach ($table[1][5] as $key => $var){
				$content .= '
					<tr>
						<td class="count">'.$count.'</td>
						<td><img src="{path}Database/icons/small/'.$table[3][5][$key][2].'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'">'.$table[3][5][$key][1].'</a></td>
						<td>'.$this->mnReadable($var).'</td>
		...
		';
			...
			foreach ($table[1][4] as $key => $var){
				$content .= '
					<tr>
						<td class="count">'.$count.'</td>
						<td><img src="{path}Database/icons/small/'.$table[3][4][$key][2].'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://tbc-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'">'.$table[3][4][$key][1].'</a></td>
						<td>'.$this->mnReadable($var).'</td>
