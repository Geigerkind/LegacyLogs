...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT a.id, a.npcid, a.cbt, a.charid,a.abilityid FROM `wotlk-raids-individual-interrupts` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
				$t[$row->id] = $row;
			}
		}else{
			$offset = explode(",", $this->raidinfo->indintm);
			foreach($this->db->query('SELECT a.amount, a.targetid, a.abilityid, a.npcid FROM `wotlk-raids-interruptsmissed` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.amount DESC') as $row){
				$t[7] += $row->amount;
			...
			$offset = explode(",", $this->raidinfo->indint);
			foreach($this->db->query('SELECT a.id, a.charid, a.abilityid, a.cbt FROM `wotlk-raids-individual-interrupts` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.cbt') as $row){
				$t[10] += 1;
				$t[11][$row->id] = $row;
	...
	public function content(){
		...
		foreach ($table as $key => $var){
			$content .= '
						<tr>
							...
							<td><img src="{path}Database/classes/c'.$this->participants[4][$var->charid]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$var->charid]->classid].'" href="{path}WOTLK/Raids/Evaluation/Interrupts/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$var->charid.'&sel='.$this->sel.'&pet='.$this->pet.'">'.$this->participants[4][$var->charid]->name.'</a></td>
							<td><img src="{path}Database/icons/small/'.$this->spells[$var->abilityid]->icon.'.jpg" /><a rel="spell='.$var->abilityid.'" href="https://wotlk-twinhead.twinstar.cz/?spell='.$var->abilityid.'" target="_blank">'.$this->spells[$var->abilityid]->name.'</a></td>
						</tr>
			';
					$count++;
		}
		...
		foreach ($table[2] as $key => $var){
			$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								<td><img src="{path}Database/classes/c'.$this->participants[4][$key]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$key]->classid].'" href="{path}WOTLK/Raids/Evaluation/Interrupts/index.php?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$key.'&sel='.$this->sel.'&pet='.$this->pet.'">'.$this->participants[4][$key]->name.'</a></td>
		...
		foreach ($table[8] as $k => $v){
			...
			$content .= '
							<tr>
								...
								<td><img src="{path}Database/classes/c'.$this->participants[4][$ke]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$ke]->classid].'" href="{path}WOTLK/Character/'.$raidinfo->sname.'/'.$this->participants[4][$ke]->name.'/0">'.$this->participants[4][$ke]->name.'</a></td>
								<td><img src="{path}Database/icons/small/'.$this->spells[$key]->icon.'.jpg" /><a rel="spell='.$key.'" href="https://wotlk-twinhead.twinstar.cz/?spell='.$this->spells[$key]->id.'" target="_blank">'.$this->spells[$key]->name.'</a></td>
								<td>'.$this->mnReadable($var).'</td>
							</tr>
			';
					$count++;
