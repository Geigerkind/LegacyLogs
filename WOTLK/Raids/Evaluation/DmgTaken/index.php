...
class Home extends Template{
		...
		switch ($this->mode){
			case 0 :
				if ($this->player){
					...
					foreach ($this->db->query('SELECT * FROM `wotlk-raids-individual-dmgtakenbyplayer` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
						$t[1] += $row->amount;
					...
					foreach ($this->db->query('SELECT * FROM `wotlk-raids-individual-dmgtakenfromsource` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
						$t[1] += $row->amount;
						$t[2][$row->charid] += $row->amount;
						$t[3][$row->charid] += $row->active;
						if (!$t[4] || $t[2][$row->charid]>$t[4])
			...
			case 1 :
				$offset = explode(",", $this->raidinfo->inddtfs);
				foreach ($this->db->query('SELECT a.npcid as typeid, a.amount, a.active, b.duration, a.attemptid FROM `wotlk-raids-individual-dmgtakenfromsource` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$t[1] += $row->amount;
			...
			case 2 : 
				...
				foreach ($this->db->query('SELECT a.abilityid, a.amount, a.casts, a.hits, a.crit, a.miss, a.parry, a.dodge, a.resist FROM `wotlk-raids-individual-dmgtakenfromability` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2.' ORDER BY a.casts') as $row){
					$t[1] += $row->amount;
	...
	private function getGraphValues(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `wotlk-raids-newbuffs` WHERE id BETWEEN '.$offset2[0].' AND '.$offset2[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `wotlk-raids-newdebuffs` WHERE id BETWEEN '.$offset2[0].' AND '.$offset2[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			if (($this->tarid && $this->mode!=0) or $this->mode==2){
				foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid FROM `wotlk-raids-graph-individual-dmgtaken` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
				...
				$con2 = '';
				foreach ($this->db->query('SELECT a.time, a.amount, a.culpritid, a.abilityid FROM `wotlk-raids-graph-individual-friendlyfire` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.npcid, a.abilityid FROM `wotlk-raids-graph-individual-dmgtaken` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
			...
			if ($this->mode==2){
				foreach ($this->db->query('SELECT a.time, a.amount, a.charid, a.abilityid FROM `wotlk-raids-graph-individual-dmgtaken` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$q[$row->abilityid][1] .= $row->time.",";
					$q[$row->abilityid][2] .= $row->amount.",";
				}
			...
			if ($this->mode==1){
				foreach ($this->db->query('SELECT a.time, a.amount, a.npcid FROM `wotlk-raids-graph-individual-dmgtaken` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.charid FROM `wotlk-raids-graph-individual-dmgtaken` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
