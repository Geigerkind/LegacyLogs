...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->mode == 0){
			...
			foreach($this->db->query('SELECT * FROM `wotlk-raids-newbuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND '.$con.'rid = '.$this->rid) as $row){
			...
			foreach($this->db->query('SELECT a.abilityid, a.amount, a.chance FROM `wotlk-raids-individual-procs` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY amount') as $row){
