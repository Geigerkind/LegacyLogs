...
class Home extends Template{
		...
		switch ($this->mode){
			case 0 :
				...
				if ($this->player){
					...
					foreach($this->db->query('SELECT a.charid, a.tamount, a.active, a.eamount, a.absorbed, a.tarid FROM `wotlk-raids-individual-healingtofriendly` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con4.' AND a.tarid = "'.$this->player.'"') as $row){
					...
					foreach ($this->db->query('SELECT a.tamount, a.eamount, a.absorbed, a.charid, a.active, a.tarid, a.attemptid, b.duration FROM `wotlk-raids-individual-healingtofriendly` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con4) as $row){
						$t[1] += $row->tamount+$row->absorbed;
						$t[2][$row->charid] += $row->tamount+$row->absorbed;
						if ($t[8][$row->charid][$row->attemptid]<=$this->atmpts[1]){
							if ($row->active>$row->duration or (($t[8][$row->charid][$row->attemptid]+$row->active)>$row->duration))
			...
			case 1 :
				$offset = explode(",", $this->raidinfo->indhtf);
				foreach ($this->db->query('SELECT a.tarid as uid, a.active, a.absorbed, a.tamount, a.eamount, a.charid, b.duration, a.attemptid FROM `wotlk-raids-individual-healingtofriendly` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con4) as $row){
					$t[1] += $row->tamount+$row->absorbed;
			...
			case 2 : 
				...
				foreach ($this->db->query('SELECT a.abilityid, a.eamount, a.tamount, a.casts, a.crit FROM `wotlk-raids-individual-healingbyability` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con4) as $row){
					$t[1] += $row->tamount;
	...
	private function getGraphValues(){
		...
		if ($this->player){
			...
			if ($this->tarid){
				if ($this->mode==0){
					foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2.$con4) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
				}else{
					foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con5) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
				}
				...
				if ($this->mode==1){
					foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid, a.charid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con5) as $row){
					...
					foreach ($this->db->query('SELECT a.time, a.amount, a.sourceid, a.abilityid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2.$con4) as $row){
			...
			if ($this->mode==2){
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.sourceid, a.abilityid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con5) as $row){
					$q[$row->sourceid][$row->abilityid][1] .= $row->time.",";
					$q[$row->sourceid][$row->abilityid][2] .= $row->amount.",";
				}	
				return $this->evalStackedGraphValues($q, true);
			...
			if ($this->mode==1){
				foreach ($this->db->query('SELECT a.time, a.amount, a.charid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con5) as $row){
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.sourceid FROM `wotlk-raids-graph-individual-healingreceived` a LEFT JOIN `wotlk-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con3.$con5) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
