...
$pdo2 = new PDO('mysql:host=localhost;dbname=world_wotlk', 'root', '', array(PDO::ATTR_EMULATE_PREPARES => false, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));

// Gettings all parsed items
$itemsNew = array();
foreach($pdo->query('SELECT * FROM `item_template-wotlk`') as $row){
...
// Getting SpellDCB
$spellDBC = array();
foreach($pdo->query('SELECT column_1, column_81, column_82, column_128, column_162 FROM spelldbc_wotlk') as $row){
...
foreach($itemsNon as $k => $v){
	...
	for ($i=1; $i<6; $i++){
		...
		if (strpos($str, "Increases spell power by")){
			setUpVal($items[$k]["spellpower"], $num);
			continue;
		}
		...
		if (strpos($str, "armor penetration rating")){
	...
	$items[$k]["socketBonus"] = $itemsNew[$k]["socketBonus"];
}

// Update this shiet!

$pdo->query('DELETE FROM `item_template-wotlk`');
$pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, true);
$sql = 'INSERT INTO `item_template-wotlk` (entry, name, icon, quality, inventorytype, armor, holy_res, fire_res, nature_res, frost_res, shadow_res, arcane_res, hit, crit, apower, dodge, parry, block, spellpower, healpower, manaregen, defense, blockchance, strength, agility, stamina, intellect, spirit, resilience, haste, armorpen, expertise, spellpen,
...
foreach($items as $k => $v){
	...
	$insertData[] = $k;
	$insertData[] = $v["name"];
	$insertData[] = (isset($v["icon"])) ? $v["icon"] : "";
	...
	$insertData[] = (isset($v["extraTooltip"])) ? $v["extraTooltip"] : "";
