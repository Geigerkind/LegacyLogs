	...
	if (isset($_GET["type"])){
		if ($_GET["type"]=="item"){
			...
			if ($itemid>0){
				...
				if ($charid>0){
					$q = $db->query('SELECT a.*, b.*, c.name as ench FROM `item_template-wotlk` a LEFT JOIN `wotlk-itemsets` b ON a.itemset = b.id LEFT JOIN `armory-enchantmentstats-wotlk` c ON a.socketBonus = c.enchid WHERE entry = "'.$itemid.'";')->fetch();
					$charitemset = $db->query('SELECT * FROM `armory-itemsets-wotlk` WHERE `index` = 0 AND charid = '.$charid)->fetch();
					$enchdb = array();
					$itemNum = 1;
					for($i=1;$i<=19;$i++){
					...
					foreach($db->query('SELECT * FROM `armory-enchantmentstats-wotlk` a LEFT JOIN `gemproperties-wotlk` b ON a.enchid = b.gemid WHERE enchid IN ('.implode(",",$gems).','.intval($charitemset->{"ench".$itemNum}).')') as $row){
					...
					if ($sockets!=""){
						...
						$enchdb = array();
						if (!empty($sockets)){
							foreach($db->query('SELECT * FROM `armory-enchantmentstats-wotlk` a LEFT JOIN `gemproperties-wotlk` b ON a.enchid = b.gemid WHERE enchid IN ('.implode(",",$gems).')') as $row){
								$enchdb[$row->enchid] = $row;
							}
						}
					}
				...
				// Effects
				$data .= '<table style="margin-bottom: 0px;"><tr><td>';
			...
			if ($spellid>0){
				...
				$keyData = new KeyData();
				$db = new Mysql($keyData->host, $keyData->user, $keyData->pass, $keyData->db, 3306, false, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
				$q = $db->query('SELECT name, icon, tooltip FROM wotlk_spells WHERE id = "'.$spellid.'";')->fetch();
				$spellTooltip = array(
					"type" => "spell",
					"entry" => "".$spellid,
					"datadisc" => "WOTLK",
