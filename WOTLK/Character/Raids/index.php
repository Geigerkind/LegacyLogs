...
class Home extends Site{
	...
	public function content(){
		...
		';
		$count = $this->db->query('SELECT a.id FROM `wotlk-raids` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN `wotlk-raids-participants` c ON a.id = c.rid WHERE (c.dps LIKE "%'.$this->info->id.'%" or c.healers LIKE "%'.$this->info->id.'%" or c.tanks LIKE "%'.$this->info->id.'%") AND a.rdy = 1')->rowCount();
		$max = floor($count/15+1);
		$i = 0;
		foreach ($this->db->query('SELECT a.tsstart, a.nameid, b.name, a.id FROM `wotlk-raids` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN `wotlk-raids-participants` c ON a.id = c.rid WHERE (c.dps LIKE "%'.$this->info->id.'%" or c.healers LIKE "%'.$this->info->id.'%" or c.tanks LIKE "%'.$this->info->id.'%") AND a.rdy = 1 ORDER BY a.id DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
			$content .= '
						<tr>
		...
		$content .= '
			...
			<footer class="pager">
				<div class="pleft">
					<a href="{path}WOTLK/Character/Raids/'.$this->sname.'/'.$this->name.'/1"><button class="icon-button icon-darrowleft"></button></a><a href="{path}WOTLK/Character/Raids/'.$this->sname.'/'.$this->name.'/'.($r = (($this->page-1)>=1) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft"></button></a>
				</div>
				<div class="pright">
					<div class="pseudoButton">Page '.$this->page.' of '.$max.'</div><a href="{path}WOTLK/Character/Raids/'.$this->sname.'/'.$this->name.'/'.($r = (($this->page+1)<=$max) ? ($this->page+1) : $max).'"><button class="icon-button icon-arrowright"></button></a><a href="{path}WOTLK/Character/Raids/'.$this->sname.'/'.$this->name.'/'.$max.'"><button class="icon-button icon-darrowright"></button></a>
				</div>
			</footer>
		</div>
		';
