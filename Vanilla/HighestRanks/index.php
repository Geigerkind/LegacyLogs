	...
	public function content(){
		...
		<div class="container cont" id="container">
			...
			$content .= '
				...
				<a href="{path}Vanilla/HighestRanks/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($this->page+1).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
				<a href="{path}Vanilla/HighestRanks/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($r = (($this->page-1)>0) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
				...
				<div class="right">
					<table cellspacing="0">
						<tr>
							...
							<th class="fstring">Date Achieved</th>
							<th class="timg char">Character</th>
							<th class="timg">Guild</th>
							<th class="fstring">Server</th>
							<th class="rank">Total HKs</th>
							<th class="fstring">Seen</th>
						</tr>
		';
		$num = (($this->page-1)*15)+1;
		foreach($this->db->query('SELECT a.liferank, a.liferanktime, b.name, c.name as gname, d.name as sname, a.lifehk, e.seen, b.faction, b.classid, e.gender, e.race FROM `armory-honor` a LEFT JOIN `chars` b ON a.charid = b.id LEFT JOIN guilds c ON b.guildid = c.id LEFT JOIN servernames d ON b.serverid = d.id LEFT JOIN `armory` e ON a.charid = e.charid'.$this->getConditions().' ORDER BY a.liferank DESC, a.liferanktime ASC LIMIT '.(($this->page-1)*15).', 15') as $row){
			$fac = ($row->faction==1) ? "alliance" : "horde";
			$content .= '
						<tr>
							<td class="rank">'.$num.'</td>
							<td class="rank pvp" style="background-image: url(\'img/pvprank'.($row->liferank-4).'.png\');"></td>
							<td class="fstring">'.date("d.m.y H:i", $row->liferanktime).'</td>
