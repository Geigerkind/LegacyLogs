...
class Home extends Site{
	...
	private function getProgress($progress){
		if ($progress>0)
			return '<span class="green">+'.$progress.'%</span>';
		if ($progress<0)
			return '<span class="red">-'.$progress.'%</span>';
		if ($progress==0)
			return '0';
	}
	...
	public function content(){
		...
		<div class="container cont" id="container">
			...
			$content .= '
				...
				<a href="{path}Vanilla/Ranks/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($this->page+1).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
				<a href="{path}Vanilla/Ranks/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($r = (($this->page-1)>0) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
				...
				<div class="right">
					<table cellspacing="0">
						<tr>
							...
							<th class="fstring">Rank Change</th>
							<th class="fstring">Seen</th>
						</tr>
		';
		$num = (($this->page-1)*15)+1;
		foreach($this->db->query('SELECT a.currank, b.name, c.name as gname, d.name as sname, a.lifehk, e.seen, b.faction, b.classid, a.lweekhonor, a.curpro, e.race, e.gender FROM `armory-honor` a LEFT JOIN `chars` b ON a.charid = b.id LEFT JOIN guilds c ON b.guildid = c.id LEFT JOIN servernames d ON b.serverid = d.id LEFT JOIN `armory` e ON a.charid = e.charid'.$this->getConditions().' ORDER BY a.currank DESC, a.lifehk DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
			...
			$content .= '
						<tr>
							...
							<td class="fstring">'.$this->getProgress($row->curpro/10).'</td>
