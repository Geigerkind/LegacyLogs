...
class Home extends Template{
	...
	private function getTable(){
		...
		foreach($this->db->query('SELECT a.charid, a.abilityid, a.amount, a.targetid, c.name, c.icon FROM `v-raids-dispels` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id LEFT JOIN spells c ON a.abilityid = c.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.($r = ($this->mode) ? 'c.type = "'.$this->mode.'" AND' : '').' '.$con.' ORDER BY a.amount DESC;') as $row){
			$t[1] += $row->amount;
				...
				foreach($va as $key => $var){
					$content .= '
							<tr>
								...
								<td><img src="{path}Database/icons/small/'.$table[5][$key][2].'.jpg" /><a rel="spell='.$key.'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$key]->realspellid.'" target="_blank">'.$table[5][$key][1].'</a></td>
								<td>'.$this->mnReadable($var).'</td>
							</tr>
					';
			$count++;
