...
class Home extends Template{
	...
	private function getLoot($aids){
		...
		foreach($this->db->query('SELECT b.id, b.loot, c.name, c.icon, c.rarity, b.charid, b.attemptid FROM `v-raids-loot` b LEFT JOIN items c ON b.loot = c.id WHERE b.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND '.$con.$con4.' ORDER BY b.id') as $q){
			if ($q->icon)
	...
	private function getTable(){
		...
		$aid = $this->db->query('SELECT group_concat(id) as aid FROM `v-raids-attempts` WHERE '.$con.' AND rdy = 1;')->fetch()->aid;
		$offsets = array();
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT * FROM `v-raids-individual-dmgdonebyability` WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND attemptid IN ('.$aid.') AND charid IN('.$this->player.$this->getPet($this->player).')'.$con2.' ORDER BY amount DESC;') as $q){
				$p[1] += $q->amount;
				$p[2][$q->abilityid] += $q->amount;
				if ($p[3]<$p[2][$q->abilityid] or !$p[3])
					$p[3] = $p[2][$q->abilityid];
			}
			foreach($this->db->query('SELECT * FROM `v-raids-individual-dmgtakenfromability` WHERE id BETWEEN '.$offsets[2][0].' AND '.$offsets[2][1].' AND attemptid IN ('.$aid.') AND charid = '.$this->player.''.$con2.' ORDER BY amount DESC;') as $q){
				$p[4] += $q->amount;
				$p[5][$q->abilityid] += $q->amount;
				if ($p[6]<$p[5][$q->abilityid] or !$p[6])
					$p[6] = $p[5][$q->abilityid];
			}
			foreach($this->db->query('SELECT * FROM `v-raids-individual-healingbyability` WHERE id BETWEEN '.$offsets[3][0].' AND '.$offsets[3][1].' AND attemptid IN ('.$aid.') AND charid = '.$this->player.$con4.' ORDER BY eamount DESC;') as $q){
				$p[7] += $q->eamount;
				$p[8][$q->abilityid] += $q->eamount;
				if ($p[9]<$p[8][$q->abilityid] or !$p[9])
					$p[9] = $p[8][$q->abilityid];
			}
			foreach($this->db->query('SELECT * FROM `v-raids-deaths` WHERE id BETWEEN '.$offsets[4][0].' AND '.$offsets[4][1].' AND attemptid IN ('.$aid.') AND charid = '.$this->player.''.$con3.';') as $q){
				$p[10][$q->id] = Array(
					1 => $q->charid,
					2 => $q->killingblow,
					3 => $q->time,
			...
			foreach($this->db->query('SELECT amount, charid FROM `v-raids-individual-dmgdonetoenemy` WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND attemptid IN ('.$aid.')'.$con2.' ORDER BY amount DESC;') as $q){
				$p[1] += $q->amount;
				if ($this->pet == 0){
			...
			foreach($this->db->query('SELECT amount, charid FROM `v-raids-individual-dmgtakenfromsource` WHERE id BETWEEN '.$offsets[2][0].' AND '.$offsets[2][1].' AND attemptid IN ('.$aid.')'.$con2.' ORDER BY amount DESC;') as $q){
				$p[4] += $q->amount;
				$p[5][$q->charid] += $q->amount;
				if ($p[6]<$p[5][$q->charid] or !$p[6])
					$p[6] = $p[5][$q->charid];
			}
			foreach($this->db->query('SELECT eamount, charid FROM `v-raids-individual-healingtofriendly` WHERE id BETWEEN '.$offsets[3][0].' AND '.$offsets[3][1].' AND attemptid IN ('.$aid.')'.$con4.' ORDER BY eamount DESC;') as $q){
				$p[7] += $q->eamount;
				$p[8][$q->charid] += $q->eamount;
				if ($p[9]<$p[8][$q->charid] or !$p[9])
					$p[9] = $p[8][$q->charid];
			}
			foreach($this->db->query('SELECT charid, id, killingblow, time, cbt FROM `v-raids-deaths` WHERE id BETWEEN '.$offsets[4][0].' AND '.$offsets[4][1].' AND attemptid IN ('.$aid.')'.$con3.' ORDER BY cbt;') as $q){
				$p[10][$q->id] = Array(
					1 => $q->charid,
					2 => $q->killingblow,
					3 => $q->time,
	...
	private function jsToAdd($aids, $lastaid){
		...
		if ($this->attempts){
			...
			//print 'SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgdone` a WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND '.$con3.$con2.$con5;
			foreach ($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgdone` a WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND '.$con3.$con2.$con5) as $row){
				$q->time .= $row->time.","; $q->amount .= $row->amount.",";
			}
			foreach ($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgtaken` a WHERE id BETWEEN '.$offsets[2][0].' AND '.$offsets[2][1].' AND '.$con3.$con.$con5) as $row){
				$q2->time .= $row->time.","; $q2->amount .= $row->amount.",";
			}
			foreach ($this->db->query('SELECT a.time, a.amount, a.sourceid FROM `v-raids-graph-individual-healingreceived` a WHERE id BETWEEN '.$offsets[3][0].' AND '.$offsets[3][1].' AND '.$con3.$con6) as $row){
			...
			foreach($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgdone` a WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND a.attemptid = '.$this->sel.$con2.$con5) as $row){
				$q->time .= $row->time.","; $q->amount .= $row->amount.",";
			}
			foreach($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgtaken` a WHERE id BETWEEN '.$offsets[2][0].' AND '.$offsets[2][1].' AND a.attemptid = '.$this->sel.$con.$con5) as $row){
				$q2->time .= $row->time.","; $q2->amount .= $row->amount.",";
			}
			foreach($this->db->query('SELECT a.time, a.amount, a.sourceid FROM `v-raids-graph-individual-healingreceived` a WHERE id BETWEEN '.$offsets[3][0].' AND '.$offsets[3][1].' AND a.attemptid = '.$this->sel.$con6) as $row){
				if (($row->sourceid==$this->tarid) or !$this->tarid)
					$q3->time .= $row->time.","; $q3->amount .= $row->amount.",";
				if ($this->player)
					$q4->time .= $row->time.","; $q4->amount .= $row->amount.",";
			...
			foreach($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgdone` a WHERE id BETWEEN '.$offsets[1][0].' AND '.$offsets[1][1].' AND a.attemptid IN ('.$aids.')'.$con2.$con5) as $row){
				$q->time .= $row->time.",";
				$q->amount .= $row->amount.",";
			}
			foreach($this->db->query('SELECT a.time, a.amount FROM `v-raids-graph-individual-dmgtaken` a WHERE id BETWEEN '.$offsets[2][0].' AND '.$offsets[2][1].' AND a.attemptid IN ('.$aids.')'.$con.$con5) as $row){
				$q2->time .= $row->time.",";
				$q2->amount .= $row->amount.",";
			}
			foreach($this->db->query('SELECT a.time, a.amount, a.sourceid FROM `v-raids-graph-individual-healingreceived` a WHERE id BETWEEN '.$offsets[3][0].' AND '.$offsets[3][1].' AND a.attemptid IN ('.$aids.')'.$con.$con6) as $row){
				if (($row->sourceid==$this->tarid) or !$this->tarid){
					$q3->time .= $row->time.",";
	...
	public function content(){
		...
		foreach($loot as $ke => $va){
			...
			foreach($va as $var){
				$content .= '
					<div class="item qe'.$var->rarity.'" style="background-image: url(\'{path}Database/icons/large/'.$var->icon.'.jpg\');">
						<a rel="item-'.$var->loot.'" href="https://vanilla-twinhead.twinstar.cz/?item='.$var->loot.'" target="_blank"><div></div></a>
						<a class="color-'.$this->classById[$this->participants[4][$var->charid]->classid].'" href="{path}Vanilla/Character/'.$this->raidinfo->sname.'/'.($r = ($this->participants[4][$var->charid]) ? $this->participants[4][$var->charid]->name : "Unknown").'/0">'.($r = ($this->participants[4][$var->charid]) ? $this->participants[4][$var->charid]->name : "Unknown").'</a>
					</div>
				';
			}
			}else{
			$content .= '
			<div class="itemgroup">
				<div class="itemgroupname"><a href="{path}Vanilla/Raids/Evaluation/?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$this->player.'&sel='.$ke.'&pet='.$this->pet.'&tarid='.$this->tarid.'">'.$this->npcsById[$this->atmpts[8][$ke]]->name.'</a></div>
		...
		foreach($this->participants[1] as $key => $var){
			if ($var->name)
				$content .= '
					<div class="user"><img src="{path}Database/classes/c'.$var->classid.'.png" /><a class="color-'.$this->classById[$var->classid].'" href="{path}Vanilla/Character/'.$this->raidinfo->sname.'/'.$var->name.'/0">'.$var->name.'</a></div>
				';
		}
		$content .= '
				</div>
		...
		foreach($this->participants[2] as $key => $var){
			if ($var->ownerid>0 && $var->name)
				$content .= '
					<div class="user" title="Owner: '.$this->participants[4][$var->ownerid]->name.'"><img src="{path}Database/classes/c'.$var->classid.'.png" /><a class="color-'.$this->classById[$var->classid].'" href="{path}Vanilla/Character/'.$this->raidinfo->sname.'/'.$var->name.'/0">'.$var->name.'</a></div>
				';
		}
		$content .= '
				</div>
		...
		$content .= '
						...
						<tfoot>
							<tr>
								...
								<th class="amount"><div>100%</div><div class="amount-sbar short"></div><div>'.$this->mnReadable($table[1]).'</div></th>
		...
		$content .= '
						...
						<tfoot>
							<tr>
								<th class="char">Total</th>
								<th class="amount"><div>100%</div><div class="amount-sbar short"></div><div>'.$this->mnReadable($table[7]).'</div></th>
		...
		foreach ($table[5] as $k => $v){
			$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								'.($r = ($this->player) ? '<td class="char"><img src="{path}Database/icons/small/'.$this->spells[$k]->icon.'.jpg"><a target="_blank" rel="spell='.$k.'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$k]->realspellid.'" target="_blank">'.$this->spells[$k]->name.'</a></td>' : '<td class="char"><img src="{path}Database/classes/c'.$this->participants[4][$k]->classid.'.png"><a class="color-'.$this->classById[$this->participants[4][$k]->classid].'" href="{path}Vanilla/Raids/Evaluation/DmgTaken/?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$k.'&sel='.$this->sel.'&pet='.$this->pet.'&tarid='.$this->tarid.'&mode=2">'.$this->participants[4][$k]->name.'</a></td>').'
								<td class="amount">
		...
		$content .= '
						...
						<tfoot>
							<tr>
								<th class="char">Total</th>
								<th class="amount"><div>100%</div><div class="amount-sbar short"></div><div>'.$this->mnReadable($table[4]).'</div></th>
		...
		foreach ($table[10] as $k => $v){
			$content .= '
							<tr>
								<td class="count">'.$count.'</td>
								<td class="char"><img src="{path}Database/classes/c'.$this->participants[4][$v[1]]->classid.'.png"><a class="color-'.$this->classById[$this->participants[4][$v[1]]->classid].'" href="{path}Vanilla/Raids/Evaluation/Deaths/?rid='.$this->rid.'&attempts='.$this->attempts.'&player='.$v[1].'&sel='.$this->sel.'&pet='.$this->pet.'">'.$this->participants[4][$v[1]]->name.'</a></td>
								<td class="amount"><img src="{path}Database/icons/small/'.$this->spells[$v[2]]->icon.'.jpg" /><a target="_blank" rel="spell='.$v[2].'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$v[2]]->realspellid.'" target="_blank">'.$this->spells[$v[2]]->name.'</a></td>
