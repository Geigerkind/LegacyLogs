...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->rid < 874){
			...
			if ($this->mode == 0){
				foreach($this->db->query('SELECT c.cat, a.abilityid, a.amount, a.active, c.name, c.icon FROM `v-raids-'.$con2.'buffs` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id LEFT JOIN spells c ON a.abilityid = c.id  WHERE b.rdy = 1 AND '.$con.' ORDER BY amount DESC') as $row){
					$t[1][$row->cat][$row->abilityid] += $row->amount;
					$t[2][$row->cat][$row->abilityid] += $row->active;
					$t[3][$row->cat][$row->abilityid] = Array(1 => $row->name, 2 => $row->icon);
					$t[10][$row->cat][$row->abilityid] = 1;
				}
			}else{
				foreach($this->db->query('SELECT c.cat, a.abilityid, a.amount, c.name, c.icon, a.chance FROM `v-raids-'.$con2.'procs` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id LEFT JOIN spells c ON a.abilityid = c.id  WHERE b.rdy = 1 AND '.$con.' ORDER BY amount') as $row){
					if ($row->cat>2)
						$row->cat = 2;
					$this->average($t[2][$row->cat][$row->abilityid], $row->chance, $t[1][$row->cat][$row->abilityid], $row->amount);
					$t[1][$row->cat][$row->abilityid] += $row->amount;
					$t[3][$row->cat][$row->abilityid] = Array(1 => $row->name, 2 => $row->icon);
				}
			}
			...
			foreach($this->db->query('SELECT * FROM `v-raids-newbuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND '.$con.'rid = '.$this->rid) as $row){
			...
			foreach($p as $k => $v){ // spell category
				foreach($v as $ke => $va){ // spellid
					if (!isset($t[1][$k][$ke]))
						$t[1][$k][$ke] = 0;
					if (!isset($t[2][$k][$ke]))
						$t[2][$k][$ke] = 0;
					if (!isset($t[3][$k][$ke]))
						$t[10][$k][$ke] = 0;
					foreach($va as $q => $s){ // charid
						$t[10][$k][$ke]++;
						foreach($s[3] as $key => $var){ // each time frame id
							if ($var and $var != ""){
								if (($var>=$this->atmpts[9] and $var<=$this->atmpts[10]) // is the starting point within the attempt
									or ($p[$k][$ke][$q][4][$key] and ($p[$k][$ke][$q][4][$key]>=$this->atmpts[9] and $p[$k][$ke][$q][4][$key]<=$this->atmpts[10])) // is the end point within the attempt
									or ($p[$k][$ke][$q][4][$key] and ($var<=$this->atmpts[9] and $p[$k][$ke][$q][4][$key]>=$this->atmpts[10])) // is the starting and end point over the attempt timeframe (1 hour buffs)
				...
				foreach($this->db->query('SELECT a.abilityid, a.amount, a.chance FROM `v-raids-individual-procs` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY amount') as $row){
				...
				if ($var>0){
					...
					$content .= '
						<tr>
							...
							<td><img src="{path}Database/icons/small/'.$this->spells[$key]->icon.'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$key]->realspellid.'">'.$this->spells[$key]->name.'</a></td>
							<td>'.$this->mnReadable($var).'</td>
							<td>'.$this->getUptime(round(100*$table[2][1][$key]/(($this->atmpts[10]-$this->atmpts[9])*$table[10][1][$key]), 2)).'%</td>
							<td>'.($r = ($this->spells[$key]->sourceid==0) ? 'Source not set. <a href="{path}Vanilla/SetSpellSource/?id='.$key.'">Set source.' : $this->sourceById[$this->spells[$key]->sourceid]).'</td>
						</tr>
					';
			$count++;
				}
		...
		';
			...
			foreach ($table[1][1] as $key => $var){
				$content .= '
					<tr>
						<td class="count">'.$count.'</td>
						<td><img src="{path}Database/icons/small/'.$table[3][1][$key][2].'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$key]->realspellid.'">'.$table[3][1][$key][1].'</a></td>
						<td>'.$this->mnReadable($var).'</td>
		...
		';
			...
			foreach ($table[1][2] as $key => $var){
				$content .= '
					<tr>
						<td class="count">'.$count.'</td>
						<td><img src="{path}Database/icons/small/'.$table[3][2][$key][2].'.jpg" /><a target="_blank" rel="spell='.$key.'" href="https://vanilla-twinhead.twinstar.cz/?spell='.$this->spells[$key]->realspellid.'">'.$table[3][2][$key][1].'</a></td>
						<td>'.$this->mnReadable($var).'</td>
