...
class Home extends Template{
	...
	private function getTable(){
		...
		if ($this->rid < 874){
			...
			if ($this->mode == 0){
				foreach($this->db->query('SELECT c.type, a.abilityid, a.amount, a.active, c.name, c.icon FROM `v-raids-'.$con2.'debuffs` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id LEFT JOIN spells c ON a.abilityid = c.id  WHERE b.rdy = 1 AND '.$con.' ORDER BY a.amount DESC') as $row){
					$t[1][$row->type][$row->abilityid] += $row->amount;
					$t[2][$row->type][$row->abilityid] += $row->active;
					$t[3][$row->type][$row->abilityid] = Array(1 => $row-> name, 2 => $row->icon);
					$t[10][$row->type][$row->abilityid] = 1;
				}
			}else{
				foreach($this->db->query('SELECT c.type, a.abilityid, a.amount, c.name, c.icon FROM `v-raids-individual-debuffsbyplayer` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id LEFT JOIN spells c ON a.abilityid = c.id  WHERE b.rdy = 1 AND '.$con.' ORDER BY a.amount') as $row){
					$this->average($t[2][$row->type][$row->abilityid], $row->active, $t[1][$row->type][$row->abilityid], $row->amount);
					$t[1][$row->type][$row->abilityid] += $row->amount;
					$t[3][$row->type][$row->abilityid] = Array(1 => $row-> name, 2 => $row->icon);
				}
			}
			...
			foreach($this->db->query('SELECT * FROM `v-raids-newdebuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND '.$con.'rid = '.$this->rid) as $row){
			...
			foreach($p as $k => $v){
				...
				foreach($this->db->query('SELECT a.abilityid, a.amount FROM `v-raids-individual-debuffsbyplayer` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.' ORDER BY a.amount') as $row){
