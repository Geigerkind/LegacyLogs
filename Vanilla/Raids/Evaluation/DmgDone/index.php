...
class Home extends Template{
	...
	private function getTable(){
		...
		switch ($this->mode){
			case 0 :
				...
				foreach ($this->db->query('SELECT a.charid, a.amount, a.active FROM `v-raids-individual-dmgdonetoenemy` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$t[1] += $row->amount;
			...
			case 1 :
				$offset = explode(",", $this->raidinfo->indddte);
				foreach ($this->db->query('SELECT a.npcid, a.amount, a.active, a.attemptid FROM `v-raids-individual-dmgdonetoenemy` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$t[1] += $row->amount;
			...
			case 2 : 
				...
				foreach ($this->db->query('SELECT a.abilityid, a.amount, a.casts, a.hits, a.crit, a.miss, a.parry, a.dodge, a.resist FROM `v-raids-individual-dmgdonebyability` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2.' ORDER BY a.casts') as $row){
					$t[1] += $row->amount;
			...
			case 3 :
				...
				foreach ($this->db->query('SELECT a.charid, a.amount, a.culpritid FROM `v-raids-individual-dmgtakenbyplayer` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
					if ($this->player){
						$t[1] += $row->amount;
		...
		if ($this->mode != 2){
			...
			foreach ($this->db->query('SELECT a.id, a.charid, a.realm, a.charid, a.realmtype, a.realmclass, b.npcid FROM `v-raids-records` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND a.type = -1 AND '.$con.' ORDER BY a.realm, a.realmtype, a.realmclass LIMIT 12') as $row){
				$t[10][$row->id] = $row;
			}
		}
		return $t;
	...
	private function getGraphValues(){
		...
		if ($this->player){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `v-raids-newbuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			foreach($this->db->query('SELECT abilityid, cbtstart, cbtend FROM `v-raids-newdebuffs` WHERE id BETWEEN '.$offset[0].' AND '.$offset[1].' AND rid = '.$this->rid.' AND charid = '.$this->player) as $row){
			...
			if ($this->tarid){
				if ($this->mode==3){
					...
					foreach ($this->db->query('SELECT a.time, a.amount, a.charid, a.abilityid FROM `v-raids-graph-individual-friendlyfire` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND culpritid = "'.$this->player.'"') as $row){
					...
					foreach ($this->db->query('SELECT a.time, a.amount, a.npcid, a.abilityid FROM `v-raids-graph-individual-dmgdone` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
					return $this->evalStackedGraphValues($q, false, $buffs, $debuffs);
				...
				if ($this->mode>1 && $this->mode!=3){
					foreach ($this->db->query('SELECT a.time, a.amount, a.charid, a.abilityid FROM `v-raids-graph-individual-dmgdone` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
						$q[$row->abilityid][1] .= $row->time.",";
						$q[$row->abilityid][2] .= $row->amount.",";
					}
					return $this->evalStackedGraphValues($q, false, $buffs, $debuffs);
			...
			$offset = explode(",", $this->raidinfo->graphdmg);
			if ($this->mode==2){
				foreach ($this->db->query('SELECT a.time, a.amount, a.abilityid FROM `v-raids-graph-individual-dmgdone` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$q[$row->abilityid][1] .= $row->time.",";
					$q[$row->abilityid][2] .= $row->amount.",";
				}
				return $this->evalStackedGraphValues($q);
			}elseif ($this->mode==1){
				foreach ($this->db->query('SELECT a.time, a.amount, a.npcid FROM `v-raids-graph-individual-dmgdone` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
				...
				$offset = explode(",", $this->raidinfo->graphff);
				foreach ($this->db->query('SELECT a.time, a.amount, a.culpritid FROM `v-raids-graph-individual-friendlyfire` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
				...
				foreach ($this->db->query('SELECT a.time, a.amount, a.charid FROM `v-raids-graph-individual-dmgdone` a LEFT JOIN `v-raids-attempts` b ON a.attemptid = b.id WHERE a.id BETWEEN '.$offset[0].' AND '.$offset[1].' AND b.rdy = 1 AND '.$con.$con2) as $row){
					$p[1] .= $row->time.",";
					$p[2] .= $row->amount.",";
			...
			if (($this->mode==0 && !$this->tarid && !$this->player) or (($this->tarid && !$this->player) && $this->mode != 2) or ($this->mode<2 && !$this->tarid && $this->player)){
				...
				foreach ($t[2] as $key => $var){
					...
					foreach($t[1] as $k => $v){
						...
						foreach($v as $ke => $va){
							if ($ke == $key){
								$tooltip .= '<tr><td>'.str_replace("'", "\'", ($r = ((!$this->tarid) && $this->mode==1) ? $this->npcsById[$k]->name : $this->spells[$k]->name)).': <\/td><th>'.$this->mnReadable($va).'<\/th><\/tr>';
								if ($activeAuras != '<tr><th colspan="2">Beneficial: <\/th><\/tr>')
									$tooltip .= $activeAuras;
								if ($activeAuras2 != '<tr><th colspan="2">Non-Beneficial: <\/th><\/tr>')
									$tooltip .= $activeAuras2;
			...
			foreach ($table[10] as $var){
				$content .= '
							<tr>
								...
								<td class="lstring"><img src="{path}Database/classes/c'.$this->participants[4][$var->charid]->classid.'.png" /><a class="color-'.$this->classById[$this->participants[4][$var->charid]->classid].'" href="{path}Vanilla/Character/'.$raidinfo->sname.'/'.$this->participants[4][$var->charid]->name.'/0">'.$this->participants[4][$var->charid]->name.'</a></td>
								<td class="lstring"><a href="{path}Vanilla/Rankings/Table/?server=0&faction=0&type=-1&tarid='.$this->tarid.'&mode=0&id='.$this->getRankingsLink($var->npcid).'">'.$this->npcsById[$var->npcid]->name.'</a></td>
							</tr>
				';
			}
			$content .= '
