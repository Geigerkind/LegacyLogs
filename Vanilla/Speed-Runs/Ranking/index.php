...
class Home extends Site{
	...
	public function content(){
		...
		$content .= '
				...
				<a href="{path}Vanilla/Speed-Runs/Ranking/?server='.$this->server.'&faction='.$this->faction.'&mode=0&id='.($r = ($this->rid>1) ? ($this->rid-1) : $this->rid).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
				<a href="{path}Vanilla/Speed-Runs/Ranking/?server='.$this->server.'&faction='.$this->faction.'&mode=0&id='.($r = ($this->rid<7) ? ($this->rid+1) : $this->rid).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
			</section>
			<section class="table">
				<div class="right">
					<table cellspacing="0">
		...
		';
		$counter = 0;
		foreach ($this->db->query('SELECT a.'.$this->prefix[1].'time as time, a.'.$this->prefix[1].'boss as boss, a.'.$this->prefix[1].'change as changed, a.'.$this->prefix[2].'raidid as raidid, c.name as sname, b.name as gname, d.tsend, b.serverid, b.faction, a.'.$this->prefix[3].'amount as aamount FROM `v-speed-runs` a LEFT JOIN guilds b ON a.guildid = b.id LEFT JOIN servernames c ON b.serverid = c.id LEFT JOIN `v-raids` d ON a.'.$this->prefix[2].'raidid = d.id WHERE a.raidnameid = '.$this->rid.' '.$this->getConditions().' ORDER BY a.'.$this->prefix[1].'time LIMIT 50') as $row){
