	...
	public function content(){
		...
		<div class="container cont" id="container">
			...
			$content .= '
				...
				<a href="{path}Vanilla/LifetimeKills/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($this->page+1).'"><button class="icon-button icon-arrowright" title="Next"></button></a>
				<a href="{path}Vanilla/LifetimeKills/?server='.$this->server.'&faction='.$this->faction.'&class='.$this->classe.'&page='.($r = (($this->page-1)>0) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft" title="Previous"></button></a>
		...
		foreach($this->db->query('SELECT a.currank, b.name, c.name as gname, d.name as sname, a.lifehk, e.seen, b.faction, b.classid, a.lweekhonor, a.lweekhk, e.gender, e.race FROM `armory-honor` a LEFT JOIN `chars` b ON a.charid = b.id LEFT JOIN guilds c ON b.guildid = c.id LEFT JOIN servernames d ON b.serverid = d.id LEFT JOIN `armory` e ON a.charid = e.charid'.$this->getConditions().' ORDER BY a.lifehk DESC LIMIT '.(($this->page-1)*15).', 15') as $row){
