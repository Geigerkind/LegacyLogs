...
$template = array();
foreach($pdo->query('SELECT Field01, Field02, Field03, Field05, Field08, Field11, Field12, Field14 FROM enchantmentdbc') as $row){
	$template[$row["Field01"]] = $row;
}

$pdo->query('DELETE FROM `armory-enchantmentstats`');

// translate dbc
$sql = 'INSERT INTO `armory-enchantmentstats` (`enchid`, `name`, `stamina`, `strength`, `intellect`, `spirit`, `agility`, `apower`, `dodge`, `blockchance`, `block`, `spellpower`, `healpower`, `rangedapower`, `hit`, `crit`, `defense`, `spellhit`, `spellcrit`, `spellshadow`, `spellfrost`, `spellfire`, `spellnature`, `spellarcane`, `spellholy`, `res_nature`, `res_holy`, `res_fire`, `res_frost`, `res_shadow`, `res_arcane`, `manaregen`, `mana`, `health`, `armor`) VALUES ';
$insertQuery = array();
$insertData = array();
...
foreach($template as $k => $v){
	if (intval($k)!=0){
		...
		$g = array();
		preg_match("/\+(\d+)\% Block/", $v["Field14"], $g);
		if (!empty($g))
			$template["blockchance"] = $g[1];
		...
		$g = array();
		preg_match("/Block Value \+(\d+)/", $v["Field14"], $g);
		if (!empty($g))
			$template["block"] = $g[1];
		...
		$g = array();
		preg_match("/\+(\d+)\% Critical Strike/", $v["Field14"], $g);
		if (!empty($g))
			$template["crit"] = $g[1];
		...
		$insertData[] = $template["rangedapower"];
		$insertData[] = $template["hit"];
		$insertData[] = $template["crit"];
		$insertData[] = $template["defense"];
		$insertData[] = $template["spellhit"];
		$insertData[] = $template["spellcrit"];
		$insertData[] = $template["spellshadow"];
		$insertData[] = $template["spellfrost"];
		$insertData[] = $template["spellfire"];
		$insertData[] = $template["spellnature"];
		$insertData[] = $template["spellarcane"];
		$insertData[] = $template["spellholy"];
