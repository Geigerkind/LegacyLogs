...
class Home extends Site{
	...
	private function getItemInfo($db){
		$this->itemInfo = $db->query('SELECT entry, icon, name, quality FROM item_template WHERE entry = "'.$this->item.'";')->fetch();
	}
	...
	public function content(){
		...
		<div class="container" id="container">
			<section class="table">
				<div class="right">
					<h1><img class="qe'.$this->itemInfo->quality.'" src="{path}Database/icons/medium/'.$this->itemInfo->icon.'.jpg" /><a class="q'.$this->itemInfo->quality.'" href="https://vanilla-twinhead.twinstar.cz/?item='.$this->itemInfo->entry.'">'.$this->itemInfo->name.'</a></h1>
					<p>
		...
		foreach($this->db->query('SELECT a.timestamp, b.name, b.classid, b.faction, c.name as gname FROM `armory-itemhistory` a LEFT JOIN chars b ON a.charid = b.id LEFT JOIN guilds c ON b.guildid = c.id WHERE b.serverid = "'.$this->serverByNames[$this->servername].'" AND item = "'.$this->item.'" ORDER BY a.id LIMIT '.(($this->page-1)*15).', 15;') as $row){
			...
			$content .= '
				<tr>
					...
					<td class="pve"><img src="{path}Database/classes/c'.$row->classid.'.png" /><a class="color-'.$this->classById[$row->classid].'" href="{path}Vanilla/Armory/'.$this->servername.'/'.$row->name.'">'.$row->name.'</a></td>
					<td><img class="facicon" src="{path}Vanilla/Armory/img/'.$fac.'.png" /><a class="'.$fac.'" href="{path}Vanilla/Guild/'.$this->servername.'/'.$row->gname.'">'.$row->gname.'</a></td>
				</tr>
			';
		}
		...
		$content .= '
			...
			<footer class="pager">
				<div class="pleft">
					<a href="{path}Vanilla/Armory/Items/'.$this->servername.'/'.$this->item.'/1"><button class="icon-button icon-darrowleft"></button></a><a href="{path}Vanilla/Armory/Items/'.$this->servername.'/'.$this->item.'/'.($r = (($this->page-1)>=1) ? ($this->page-1) : 1).'"><button class="icon-button icon-arrowleft"></button></a>
				</div>
				<div class="pright">
					<div class="pseudoButton">Page '.$this->page.' of '.$max.'</div><a href="{path}Vanilla/Armory/Items/'.$this->servername.'/'.$this->item.'/'.($r = (($this->page+1)<=$max) ? ($this->page+1) : $max).'"><button class="icon-button icon-arrowright"></button></a><a href="{path}Vanilla/Armory/Items/'.$this->servername.'/'.$this->item.'/'.$max.'"><button class="icon-button icon-darrowright"></button></a>
				</div>
			</footer>
		</div>
		';
