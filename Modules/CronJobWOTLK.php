...
class CronJob{
	...
	private function getNewOldestFile($active){
		...
		if ((time()-filemtime($this->fileGzip))>=60 && filemtime($this->fileGzip) >= 10){
			...
			fclose($out_file);
			$savedFiles = glob('SaveWOTLK/*.lua.gz');
			foreach($savedFiles as $sf){
				if ((time()-filemtime($sf))>259200)
					unlink($sf);
			}
			copy($this->fileGzip, 'SaveWOTLK/'.str_replace("FilesWOTLK/", "", $this->fileGzip));
	...
	function getActive($db){
		...
		foreach($db->query('SELECT id, name, date FROM cronjob WHERE active = 1 AND expansion = 2') as $row){
