...
class Armory{
	...
	public function __construct($file, $db, $id){
		...
		$db->query('DELETE FROM `armory-itemsets-wotlk`');
		$db->query('DELETE FROM `armory-guildhistory`');
		$db->query('DELETE FROM `armory-arenateams-wotlk`');
		//*/
		
		//Get Itemtemplate
		$itemTemplate = array();
		foreach($this->db->query('SELECT * FROM `item_template-wotlk`') as $row){
		...
		foreach($this->arr as $k => $v){
			for ($i=9; $i<28; $i++){
				...
				preg_match("/\|(.+)\|Hitem:(\d+):(\d+):(.+):80\|h\[(.+)\]\|h\|r/", $v[$i], $g);
				$this->arr[$k][$i] = $g;
			}
		}
		...
		$arenaTeams = array();
		foreach($db->query('SELECT * FROM `armory-arenateams-wotlk` WHERE serverid = '.$serverid) as $row){
		...
		$sql = 'INSERT INTO `armory-arenateams-wotlk` (type, name, rating, games, wins, time, val, serverid,season) VALUES ';
		$insertQuery = array();
		$insertData = array();
				...
				if (isset($v[37]) && strlen($v[37])>3){
					if (isset($arenaTeams[0][$v[37]])){
						if (isset($v["season"]) && intval($v["season"])>intval($arenaTeams[0][$v[37]]->season)){
							$db->query('UPDATE `armory-arenateams-wotlk` SET season = "'.intval($v["season"]).'", rating = "'.$v[39].'", games = "'.$v[40].'", wins = "'.$v[41].'", time = "'.time().'", val = "'.$v[39].'" WHERE arenaid = '.$arenaTeams[0][$v[37]]->arenaid);
						}else{
							$db->query('UPDATE `armory-arenateams-wotlk` SET rating = "'.$v[39].'", games = "'.$v[40].'", wins = "'.$v[41].'", time = "'.($arenaTeams[0][$v[37]]->time.",".time()).'", val = "'.($arenaTeams[0][$v[37]]->val.",".$v[39]).'" WHERE arenaid = '.$arenaTeams[0][$v[37]]->arenaid);
						}
					}else{
						$insertQuery[] = '(?,?,?,?,?,?,?,?,?)';
						$insertData[] = 0;
				...
				if (isset($v[42]) && strlen($v[42])>3){
					if (isset($arenaTeams[1][$v[42]])){
						if (isset($v["season"]) && intval($v["season"])>intval($arenaTeams[1][$v[42]]->season)){
							$db->query('UPDATE `armory-arenateams-wotlk` SET season = "'.intval($v["season"]).'", rating = "'.$v[44].'", games = "'.$v[45].'", wins = "'.$v[46].'", time = "'.time().'", val = "'.$v[44].'" WHERE arenaid = '.$arenaTeams[1][$v[42]]->arenaid);
						}else{
							$db->query('UPDATE `armory-arenateams-wotlk` SET rating = "'.$v[44].'", games = "'.$v[45].'", wins = "'.$v[46].'", time = "'.($arenaTeams[1][$v[42]]->time.",".time()).'", val = "'.($arenaTeams[1][$v[42]]->val.",".$v[44]).'" WHERE arenaid = '.$arenaTeams[1][$v[42]]->arenaid);
						}
					}else{
						$insertQuery[] = '(?,?,?,?,?,?,?,?,?)';
						$insertData[] = 1;
				...
				if (isset($v[47]) && strlen($v[47])>3){
					if (isset($arenaTeams[2][$v[47]])){
						if (isset($v["season"]) && intval($v["season"])>intval($arenaTeams[2][$v[47]]->season)){
							$db->query('UPDATE `armory-arenateams-wotlk` SET season = "'.intval($v["season"]).'", rating = "'.$v[49].'", games = "'.$v[50].'", wins = "'.$v[51].'", time = "'.time().'", val = "'.$v[49].'" WHERE arenaid = '.$arenaTeams[2][$v[47]]->arenaid);
						}else{
							$db->query('UPDATE `armory-arenateams-wotlk` SET rating = "'.$v[49].'", games = "'.$v[50].'", wins = "'.$v[51].'", time = "'.($arenaTeams[2][$v[47]]->time.",".time()).'", val = "'.($arenaTeams[2][$v[47]]->val.",".$v[49]).'" WHERE arenaid = '.$arenaTeams[2][$v[47]]->arenaid);
						}
					}else{
						$insertQuery[] = '(?,?,?,?,?,?,?,?,?)';
						$insertData[] = 2;
		...
		$honor = array();
		foreach($db->query('SELECT * FROM `armory-honor-wotlk`') as $row){
		...
		$sql = 'INSERT INTO `armory-honor-wotlk` (charid, arena1, arena2, arena3, thk, thonor, yhk, yhonor, lhk) VALUES ';
		$insertQuery = array();
		$insertData = array();
		$toDeleteArray = array();
		foreach($this->arr as $k => $v){
		...
		if (!empty($insertQuery)) {
			$this->db->query('DELETE FROM `armory-honor-wotlk` WHERE charid IN ('.implode(', ', $toDeleteArray).');');
			$sql .= implode(', ', $insertQuery);
			$stmt = $this->db->prepare($sql);
			$stmt->execute($insertData);
		}
		...
		foreach($this->arr as $k => $v){
			if (isset($chars[$this->serverByName[$v[8]]][$k])){
				if ((isset($v[10][2]) && $v[10][2] != 0) || (isset($v[16][2]) && $v[16][2] != 0) || (isset($v[23][2]) && $v[23][2] != 0)){
					$itemsets = array();
					foreach($db->query('SELECT * FROM `armory-itemsets-wotlk` WHERE charid = "'.$chars[$this->serverByName[$v[8]]][$k][2].'";') as $row){
		...
		if (!empty($delete)){
			$db->query('DELETE FROM `armory-itemsets-wotlk` WHERE charid IN ('.implode(",",$delete).')');
		}
		if (!empty($insertQuery)) {
			$sql .= implode(', ', $insertQuery);
			$stmt = $this->db->prepare($sql);
